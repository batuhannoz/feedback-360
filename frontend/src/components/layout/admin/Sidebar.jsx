...
import defaultLogo from '../../../assets/icons/img.png';
import CompanyService from "../../../services/companyService.js";
import {useSelector} from "react-redux";

const Sidebar = () => {
    const { logoUrl } = useSelector((state) => state.auth);


    return (
        <div className="w-64 bg-white shadow-md">
            <div className="flex justify-center p-4 border-b">
                <img className="h-10" src={logoUrl || defaultLogo} alt="Şirket Logosu"/>
            </div>
            <nav className="mt-4">
                <h2 className="px-4 text-xs text-gray-500 uppercase tracking-wider">Genel</h2>
                <NavLink to="/dashboard" className="flex items-center px-4 py-2 mt-2 text-gray-700 hover:bg-gray-200"
                ...
                <NavLink to="/dashboard/competencies"
                         className="flex items-center px-4 py-2 mt-2 text-gray-700 hover:bg-gray-200"
                         activeClassName="bg-gray-300">
